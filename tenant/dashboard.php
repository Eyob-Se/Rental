...
<div class="house-card">
    <?php if (!empty($house['image_path'])): ?>
        <img src="../uploads/house_images/<?= htmlspecialchars($house['image_path']) ?>" alt="House Image" style="width:100%; max-height:200px; object-fit:cover; border-radius:6px; margin-bottom:10px;">
    <?php else: ?>
        <div style="width:100%; height:200px; background:#444; border-radius:6px; margin-bottom:10px; display:flex; align-items:center; justify-content:center; color:#aaa;">
            No image available
        </div>
    <?php endif; ?>
    <h3><?= htmlspecialchars($house['title']) ?></h3>
    <p><strong>Location:</strong> <?= htmlspecialchars($house['location']) ?></p>
    <p><strong>Bedrooms:</strong> <?= (int)$house['bedrooms'] ?> | <strong>Bathrooms:</strong> <?= (int)$house['bathrooms'] ?></p>
    <p><strong>Area:</strong> <?= htmlspecialchars($house['area']) ?> sq ft</p>
